...
/// A registry of clients currently connected to this node.

pub struct RegisteredClient {
    pub uaid: Uuid, // The user's primary key
    pub uid: Uuid,  // the local unique identifier
                    // pub tx: mpsc::UnboundedSender<ServerNotification>,  // channel to the websocket.
}

#[derive(Default)]
pub struct ClientRegistry {
    clients: RwLock<HashMap<Uuid, RegisteredClient>>, // list of known clients for this node.
}
...
impl ClientRegistry {
    ...
    pub async fn connect(&self, client: RegisteredClient) -> Result<()> {
        ...
        Box::new(
            self.clients
                ...
                .map_err(|_| ApiError::from("clients lock poisoned")),
        )
    ...
    pub fn notify(&self, uaid: Uuid, notif: Notification) -> MySendFuture<()> {
        let fut = self
            ...
            .map_err(|_| ApiError::from("User not connected"));
        Box::new(fut)
    }
