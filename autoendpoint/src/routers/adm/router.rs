...
impl Router for AdmRouter {
    ...
    async fn route_notification(&self, notification: &Notification) -> ApiResult<RouterResponse> {
        ...
        if new_registration_id != registration_id {
            ...
            self.db_client.update_user(&user).await?;
        }
...
mod tests {
    ...
    fn make_router(db_client: Box<dyn DbClient>) -> AdmRouter {
        AdmRouter::new(
            AdmSettings {
    ...
    async fn successful_routing_no_data() {
        ...
        let router = make_router(db_client);
    ...
    async fn no_adm_user() {
        ...
        let router = make_router(db_client.into_boxed_arc());
        let _token_mock = mock_token_endpoint();
