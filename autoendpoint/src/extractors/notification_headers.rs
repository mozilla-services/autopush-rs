...
use autopush_common::{util::InsertOpt, MAX_CHANNEL_TTL};
...
impl NotificationHeaders {
    ...
    pub fn from_request(req: &HttpRequest, has_data: bool) -> ApiResult<Self> {
        ...
        let ttl = get_header(req, "ttl")
            ...
            .map(|ttl| min(ttl, MAX_CHANNEL_TTL as i64))
...
mod tests {
    ...
    use autopush_common::MAX_CHANNEL_TTL;
    ...
    fn maximum_ttl() {
        let req = TestRequest::post()
            .insert_header(("TTL", (MAX_CHANNEL_TTL + 1).to_string()))
            .to_http_request();
        let result = NotificationHeaders::from_request(&req, false);

        assert!(result.is_ok());
        assert_eq!(result.unwrap().ttl, MAX_CHANNEL_TTL as i64);
    }
