...
* `grpc_channel_count` the number of shared tonic channels per process. Each connected channel owns one Bigtable HTTP/2 connection and supports up to 100 concurrent streams. It defaults to sixteen, which keeps wire capacity (1,600 streams) above any plausible `database_pool_max_size`, so requests wait at the visible, budget-bounded permit gate instead of parking in hyper's invisible dispatch queue during a reconnect storm. Size it from measured peak concurrency as `ceil(peak database.ops.inflight / 25)`, not from the maximum logical operation-pool size.
