...
> _**Note**_: The docker images used take approximately 400 MB of disk-space, make
...
5. Modify the environment variables set in the `docker-compose.yml` file:
    -  Run `make gen-key` (see [Generate a Crypto-key](#generate-a-crypto-key)) and replace the values of `AUTOCONNECT__CRYPTO_KEY`, `AUTOEND__CRYPTO_KEYS` and `AUTOEND__AUTH_KEYS` with the generated values. If you don't have [Poetry](https://python-poetry.org/) installed on your host machine, you can either execute the command inside the docker container or run `make install_poetry` first to install it.
    ...
    - For more configuration options, please see [Configuration options](config_options.md).
