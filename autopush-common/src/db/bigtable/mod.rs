...
// Kept strictly under the 30s GCP backend-service default `timeoutSec`, so a
// stalled scan returns our own error before the load balancer gives up on the
// request. At exactly 30s the two race and the client sees an LB 5xx instead,
// which would also hide this budget from any p95 measured on inbound requests.
const DEFAULT_GRPC_SCAN_TOTAL_TIMEOUT: Duration = Duration::from_secs(25);
